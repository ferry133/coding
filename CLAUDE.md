...
## issue／PR 的生命週期（2026-09-03 裁定）

| 誰 | 做什麼 |
|---|---|
| **repo 擁有者** | 解決自己 repo 的 issue、合併自己 repo 的 PR |
| **repo 擁有者** | 修好之後**通知開單者** |
| **開單者** | **驗收**，並把驗收結果**回報給擁有者** |
| 雙方 | 有落差就討論、調整修法，直到對「修完了沒有」有共識 |

- **修正範圍的擴張最多三次。** 第三次之後仍未收斂，**升給 ferry133 裁**，不要繼續往下長
  ——一條沒有上限的修正鏈與一個沒有人負責的 issue，從外面看是一樣的。
- **達成共識後，擁有者可以自行關 issue／合併 PR**；判斷不了就**升給 ferry133 定案**。

> ⚠️ **這一條動到了下面那句授權邊界，界線由我讀出來的，寫在這裡以便被糾正。**
> 「不要要求 peer 做需要你授權的動作（推到會即刻生效的分支）」原本使**每一次**推 main
> 都停在人點頭之前。新規則把**合併權**給了擁有者，我讀成**只在這條路徑上**：
> issue → 擁有者修 → 開單者驗收 → 共識 → 合併。
>
> **不在這條路徑上的仍然要點頭**：與任何 issue 無關的直接推送、破壞性操作、
> 以及**修改的內容超出原 issue 範圍**的合併。理由是量到的而不是保守——`jg-base` 的
> `main` 由 Flux 以 1 小時間隔套用到**每一座叢集**，所以在那個 repo 上「合併」與
> 「對全部叢集部署」是同一個動作。**若這個讀法太窄，請直接改這一段。**
