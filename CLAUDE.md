...
- **按下合併的人，要在那張 PR 上留一行署名**（`peer name [ref]`）——2026-09-13 ferry133 裁定。
  ...
  「誰合併的」進入可查紀錄的動作。**已實測有效**：`jgct#110`／`#111` 事後補了具名紀錄，
  現在指得出人；同日的 `#113`／`#115`／`#116` 沒有，**永遠指不出**。
...
>
> **不用點頭**：只服務本 repo 開發流程、不隨出貨產生任何差異的——`.github/`、
> `scripts/tests/`、`openspec/`、純註解、只在本 repo CI 跑的守衛。
>
